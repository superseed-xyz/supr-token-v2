...
contract UnitLobsterNames is Base {
  function testName() public {
    assertEq(_token.name(), 'Lobsters');
  }

  function testSymbol() public {
    assertEq(_token.symbol(), 'BUILD');
  }
...
contract UnitLobsterMintBurn is Base {
  ...
  function testMintRevertsToSelf(
    ...
    _token.setLimits(_bridge, _amount, 0);
    vm.prank(_bridge);
    vm.expectRevert(abi.encodeWithSelector(LobsterToken.LobsterToken_InvalidReceiver.selector, address(_token)));
...
contract UnitLobsterLockbox is Base {
...
contract UnitLobsterRateLimits is Base {
...
contract UnitLobsterPermit is Base {
...
contract UnitLobsterIntrospection is Base {
...
contract UnitLobsterTransferReceiverGuard is Base {
  function _fund(
    uint256 _amount
  ) internal {
    vm.prank(_owner);
