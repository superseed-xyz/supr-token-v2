...
contract MockLobsterBase is ERC20 {
  constructor() ERC20('Base BUILD', 'bBUILD') {}
}
...
abstract contract Base is Test {
  ...
  LobsterTokenFactory internal _factory;
  MockLobsterBase internal _base;

  function setUp() public virtual {
    _factory = new LobsterTokenFactory();
    _base = new MockLobsterBase();
  }
...
contract UnitLobsterFactoryDeployXERC20 is Base {
  function testTransfersOwnershipToOwnerNotDeployer() public {
    (uint256[] memory _ml, address[] memory _br) = _empty();

    vm.prank(_deployer);
    address _token = _factory.deployXERC20('Lobsters', 'BUILD', _ml, _ml, _br, _governor);

    assertEq(LobsterToken(_token).owner(), _governor);
    assertTrue(LobsterToken(_token).owner() != _deployer);
    assertEq(LobsterToken(_token).FACTORY(), address(_factory));
  }
  ...
  function testConfiguresBridgeLimits(
    ...
    vm.prank(_deployer);
    address _token = _factory.deployXERC20('Lobsters', 'BUILD', _ml, _bl, _br, _governor);

    assertEq(LobsterToken(_token).mintingMaxLimitOf(_bridge), _mint);
    assertEq(LobsterToken(_token).burningMaxLimitOf(_bridge), _burn);
  }
  ...
  function testRevertsOnLengthMismatch() public {
    ...
    _factory.deployXERC20('Lobsters', 'BUILD', _ml, _bl, _br, _governor);
  }
  ...
  function testRedeployingSameSaltReverts() public {
    ...
    _factory.deployXERC20('Lobsters', 'BUILD', _ml, _ml, _br, _governor);
  ...
  function testRevertsOnZeroOwner() public {
    ...
    _factory.deployXERC20('Lobsters', 'BUILD', _ml, _ml, _br, address(0));
  }
}
...
contract UnitLobsterFactoryDeployWithLockbox is Base {
  function testDeploysTokenAndLockboxAndTransfersOwnership() public {
    (uint256[] memory _ml, address[] memory _br) = _empty();

    vm.prank(_deployer);
    (address _token, address payable _lockbox) =
      _factory.deployXERC20WithLockbox('Lobsters', 'BUILD', _ml, _ml, _br, address(_base), false, _governor);

    assertEq(LobsterToken(_token).owner(), _governor);
    assertEq(LobsterToken(_token).lockbox(), _lockbox);
  ...
  function testRevertsOnBadTokenAddress() public {
    ...
    _factory.deployXERC20WithLockbox('Lobsters', 'BUILD', _ml, _ml, _br, address(0), false, _governor);
  }
  ...
  function testNativeLockbox() public {
    ...
    vm.prank(_deployer);
    (address _token, address payable _lockbox) =
      _factory.deployXERC20WithLockbox('Lobsters', 'BUILD', _ml, _ml, _br, address(0), true, _governor);
...
contract UnitLobsterFactoryDeployLockboxStandalone is Base {
  ...
  function setUp() public override {
    ...
    (uint256[] memory _ml, address[] memory _br) = _empty();
    vm.prank(_deployer);
    _token = _factory.deployXERC20('Lobsters', 'BUILD', _ml, _ml, _br, _governor);
  }
