...
/// @dev Upper bound for fuzzed mint amounts; not a contract-enforced cap.
uint256 constant FUZZ_MAX = 10_000_000_000e18;

/// @dev Drives random mint/burn/transfer/setLimits against a LobsterToken from a fixed set
...
contract LobsterTokenHandler is Test {
  LobsterToken internal _token;
...
contract LobsterTokenInvariants is Test {
  LobsterToken internal _token;
  LobsterTokenHandler internal _handler;
  ...
  function setUp() public {
    ...
    _handler = new LobsterTokenHandler(_token, _owner, _bridges, _actors);

    // Restrict fuzzing to the handler's own actions (not forge-std inherited functions).
    bytes4[] memory _selectors = new bytes4[](5);
    _selectors[0] = LobsterTokenHandler.mint.selector;
    _selectors[1] = LobsterTokenHandler.burn.selector;
    _selectors[2] = LobsterTokenHandler.transfer.selector;
    _selectors[3] = LobsterTokenHandler.transferToForbidden.selector;
    _selectors[4] = LobsterTokenHandler.setLimits.selector;
