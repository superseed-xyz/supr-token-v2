 ...
 * @notice Deploys SUPRTokenV2 directly. Name and symbol are baked into the contract, so the
 ...
 *
 * Usage:
 *   forge script solidity/scripts/SUPRTokenV2Deploy.sol --account <name> --rpc-url <rpc>
 *   add --broadcast --verify to broadcast.
 */
contract SUPRTokenV2Deploy is Script {
  error SUPRTokenV2Deploy_GovernorNotSet();

  function run() public {
    address _governor = vm.envAddress('GOVERNOR_ADDRESS');
    if (_governor == address(0)) revert SUPRTokenV2Deploy_GovernorNotSet();
    ...
    SUPRTokenV2 _token = new SUPRTokenV2(msg.sender);
    ...
    // solhint-disable-next-line no-console
    console.log('SUPRTokenV2:', address(_token));
