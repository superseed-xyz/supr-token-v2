 ...
 * @notice Deploys LobsterToken (and optionally a lockbox) on every chain listed in the
 ...
 *
 * Usage:
 *   1. Fill in solidity/scripts/lobster-token-deployment-config.json
 *   2. Set LOBSTER_FACTORY_ADDRESS and all RPC env vars in .env; import the deployer keystore
 *      once with: cast wallet import <name> --interactive
 *   3. Dry-run:  forge script solidity/scripts/LobsterTokenDeploy.sol --account <name>
 *   4. Broadcast: add --broadcast --verify to the command above
 *
 * The factory must already be deployed on every target chain.
 * Run LobsterTokenFactoryDeploy.sol first if it is not.
 */
contract LobsterTokenDeploy is Script, ScriptingLibrary {
  using stdJson for string;

  error LobsterTokenDeploy_FactoryAddressNotSet();
  error LobsterTokenDeploy_GovernorNotSet();
  error LobsterTokenDeploy_NoFactoryOnChain();
  error LobsterTokenDeploy_BridgeAddressNotSet();
  error LobsterTokenDeploy_AddressMismatchAcrossChains();

  LobsterTokenFactory public factory = LobsterTokenFactory(vm.envAddress('LOBSTER_FACTORY_ADDRESS'));

  function run() public {
    if (address(factory) == address(0)) revert LobsterTokenDeploy_FactoryAddressNotSet();

    string memory _json = vm.readFile('./solidity/scripts/lobster-token-deployment-config.json');
    ...
    for (uint256 i; i < _chainAmount; i++) {
      ...
      if (_chainDetails.governor == address(0)) revert LobsterTokenDeploy_GovernorNotSet();
      ...
      if (keccak256(address(factory).code) == keccak256(address(0).code)) {
        revert LobsterTokenDeploy_NoFactoryOnChain();
      }
      ...
      for (uint256 _b; _b < _bridgeDetails.length; _b++) {
        if (_bridgeDetails[_b].bridge == address(0)) revert LobsterTokenDeploy_BridgeAddressNotSet();
    ...
    // Runtime bytecode intentionally not compared: LobsterToken inherits EIP712 (OZ v4.9.3),
    ...
    if (_deployCount > 1) {
      for (uint256 i = 1; i < _deployCount; i++) {
        if (_tokens[i - 1] != _tokens[i]) revert LobsterTokenDeploy_AddressMismatchAcrossChains();
      }
    }
  }
}
