 ...
 * When conversion is complete, the governor calls setLimits(converter, 0, 0) to close it.
 */
contract SUPRConverterDeploy is Script {
  ...
  function run() public {
    ...
    vm.startBroadcast();
    SUPRConverter _converter = new SUPRConverter(oldSupr, newSupr);
    vm.stopBroadcast();
    ...
    bytes memory _setLimitsNew = abi.encodeCall(IXERC20.setLimits, (address(_converter), OLD_TOTAL_SUPPLY, 0));
    ...
    // solhint-disable-next-line no-console
    console.log('To close conversion: owners call setLimits(converter, 0, 0) on both tokens');
  }
}
